...
class Vamtam_Customize_Color_Row_Control extends Vamtam_Customize_Control {
	...
	public function enqueue() {
		...
		$color_row_translation_array = array(
			'tooltip_msg'      => esc_html__( 'Contrasting color may be hard for people to read. Try different colors with a higher brightness difference between this accent and the associated contrasting color.', 'jolie' ),
			'tooltip_msg_auto' => esc_html__( "It's not possible to generate a color with high enough contrast given your chosen accent color. Consider changing accent color so that it is easier for people to read.", 'jolie' ),
		);
		...
		<# if ( data.id == 'vamtam_theme[accent-color]' && data.with_hc ) { #>
			...
			<div class="vamtam-color-row-container">
				...
				<div class="vamtam-color-row-single-color-wrapper">
					<span class="customize-control-title"> <?php echo esc_html__( 'Main Color', 'jolie' ); ?> </span>
				</div>
				<div class="color-row-warning"> </div>
				<div class="vamtam-color-row-single-color-wrapper">
					<span class="customize-control-title"> <?php echo esc_html__( 'Contrast Color', 'jolie' ); ?> </span>
				</div>
			</div>
			...
			<div class="auto-contrast">
				<label>
					...
					<?php esc_html_e( 'Use automatic contrast colors', 'jolie' ) ?>
				</label>
			</div>
			<# } #>
