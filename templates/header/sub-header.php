...
if ( ! is_404() ) {
	...
	if ( vamtam_has_woocommerce() ) {
		// If we are on a WC page, check if the relevant theme mod is active first.
		if ( vamtam_should_include_wc_theme_mods_for_page() ) {
			if ( is_woocommerce() && ! is_single() ) {
				if ( is_product_category() ) {
					...
					$page_title = sprintf( esc_html__( 'Search Results for: %s', 'jolie' ), '<span>' . get_search_query() . '</span>' );
				} else {
				...
				$complete_title = esc_html__( 'Order Complete', 'jolie' );
