...
function vamtam_register_required_plugins() {
	...
	$plugins = array(
		...
		array(
			'name'     => esc_html__( 'WooCommerce', 'jolie' ),
			'slug'     => 'woocommerce',
			'required' => true,
			'category' => 'required',
		),
		...
		array(
			'name'     => esc_html__( 'PW WooCommerce Gift Cards', 'jolie' ),
		...
		array(
			'name'     => esc_html__( 'Smash Balloon Social Photo Feed', 'jolie' ),
			'slug'     => 'instagram-feed',
			'required' => true,
			'category' => 'required',
		),
		...
		array(
			'name'     => esc_html__( 'Elementor', 'jolie' ),
			'slug'     => 'elementor',
			'required' => true,
			'category' => 'required',
			'version'  => '2.7.4',
		...
		array(
			'name'     => esc_html( sprintf( __( 'Vamtam Elementor Integration (%s)', 'jolie' ), VAMTAM_THEME_SLUG ) ),
		...
		array(
			'name'     => esc_html__( 'Vamtam Importers (E)', 'jolie' ),
		...
		array(
			'name'     => esc_html__( 'Wordpress SEO', 'jolie' ),
			'slug'     => 'wordpress-seo',
			'required' => false,
		...
		array(
			'name'     => esc_html__( 'Really Simple SSL', 'jolie' ),
		...
		array(
			'name'     => esc_html__( 'Loco Translate', 'jolie' ),
			'slug'     => 'loco-translate',
			'required' => false,
			'category' => 'recommended',
		),
		...
		array(
			'name'     => esc_html__( 'UpDraftPlus', 'jolie' ),
			'slug'     => 'updraftplus',
			'required' => false,
			'category' => 'recommended',
		),
		...
		array(
			'name'     => esc_html__( 'WP Super Cache', 'jolie' ),
			'slug'     => 'wp-super-cache',
			'required' => false,
			'category' => 'recommended',
		),
		...
		array(
			'name'     => esc_html__( 'Limit Login Attempts Reloaded', 'jolie' ),
		...
		array(
			'name'     => esc_html__( 'Wordfence', 'jolie' ),
			'slug'     => 'wordfence',
			'required' => false,
			'category' => 'recommended',
		),
		...
		array(
			'name'     => esc_html__( 'CookieYes | GDPR Cookie Consent & Compliance Notice (CCPA Ready)', 'jolie' ),
...
function vamtam_tgmpa_table_columns( $columns ) {
	...
	$columns = array(
		...
		'img'         => '',
		'plugin'      => esc_html__( 'Name', 'jolie' ),
		'description' => esc_html__( 'Description', 'jolie' ),
		'status'      => esc_html__( 'Status', 'jolie' ),
		'version'     => esc_html__( 'Version', 'jolie' ),
	);
	return $columns;
}
...
function vamtam_tgmpa_table_data_item( $item, $plugin )
{
	...
	if( isset( $plugin_data['Description'] ) ) {
		...
		$item['description'] =  esc_html__( 'A simple WordPress Plugin.', 'jolie' );
	}
	if ( strpos( $plugin['slug'], 'jolie' ) !== false ) {
		$item['description'] =  esc_html__( 'Theme Exclusive.', 'jolie' );
	}
...
function vamtam_tgmpa_admin_menu_args( $args ) {
	$args['page_title'] = esc_html__( 'Install Plugins', 'jolie' );
