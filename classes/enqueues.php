...
class VamtamEnqueues {
	...
	public static function scripts() {
		...
		wp_register_script( 'vamtam-wc-gallery', VAMTAM_JS . 'wc-gallery' . ( self::$use_min ? '.min' : '' ) . '.js', [ 'vamtam-all' ], $cache_timestamp, ['strategy' => 'defer'] );
	}
	...
	public static function admin_scripts() {
		...
		wp_localize_script(
			'vamtam-admin', 'VAMTAM_ADMIN', array(
				'addNewIcon' => esc_html__( 'Add New Icon', 'jolie' ),
				'iconName'   => esc_html__( 'Icon', 'jolie' ),
				'iconText'   => esc_html__( 'Text', 'jolie' ),
				'iconLink'   => esc_html__( 'Link', 'jolie' ),
				'iconChange' => esc_html__( 'Change', 'jolie' ),
				'fonts'      => $GLOBALS['vamtam_fonts'],
				'home_url'   => home_url(),
				'elementsTxt' => esc_html__( 'Enter your Envato Elements Token.', 'jolie' ),
				'tfPcTxt' => esc_html__( 'Enter your purchase code from ThemeForest to receive theme updates and support.', 'jolie' ),
				'unRegPcTxt' => esc_html__( 'You are about to unregister your purchase code. Are you sure you want to continue?', 'jolie' ),
				'unRegTokenTxt' => esc_html__( 'You are about to unregister your Envato Elements Token. Are you sure you want to continue?', 'jolie' ),
	...
	public static function enqueue_elementor_styles() {
		...
		if ( $is_wc_page && ! $built_with_elementor ) {
			...
			if ( is_cart() || is_checkout() || is_account_page() ) {
				...
				if ( $wc_mod_active ) {
					// WC Page-specific styles.
					wp_register_style( "vamtam-theme-elementor-wc-{$wc_page}", VAMTAM_ASSETS_URI . "css/dist/elementor/woocommerce/{$wc_page}/elementor-wc-{$wc_page}-all.css", array(), $cache_timestamp );
					wp_enqueue_style( "vamtam-theme-elementor-wc-{$wc_page}" );

					// Responsive WC Page-specific styles.
					$responsive_wc_stylesheets = array(
						"elementor-wc-{$wc_page}-max"      => "(min-width: {$medium_breakpoint}px)",
						"elementor-wc-{$wc_page}-max-low"   => "(min-width: {$medium_breakpoint}px) and (max-width: " . ( $content_width + 80 ) . "px)", // (we add 80px to maintain limit wrapper's 40left + 40right padding in the 1260-1340px screen size)
						"elementor-wc-{$wc_page}-below-max" => '(max-width: ' . ( $medium_breakpoint - 1 ) . 'px)',
						"elementor-wc-{$wc_page}-small"    => '(max-width: ' . ( $small_breakpoint - 1 ) . 'px)',
					);

					$url_prefix  = VAMTAM_ASSETS_URI . "css/dist/elementor/woocommerce/{$wc_page}/responsive/";
					$file_prefix = VAMTAM_ASSETS_DIR . "css/dist/elementor/woocommerce/{$wc_page}/responsive/";

					foreach ( $responsive_wc_stylesheets as $file => $media ) {
						if ( file_exists( $file_prefix . $file . '.css' ) ) {
							wp_enqueue_style( 'vamtam-theme-'. $file, $url_prefix . $file . '.css', array( "vamtam-theme-elementor-wc-{$wc_page}" ), $cache_timestamp, $media );
						}
					}
				}
			}
