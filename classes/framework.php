...
class VamtamFramework {
	...
	public static function theme_supports() {
		...
		function vamtam_elementor_widgets_support( bool $supports, array $args, string $feature ) {
			...
			$base_features = [
				...
				'image--grow-with-scale-anims',					  // Lagar-after.
			...
			$theme_features = [
				...
				'woocommerce-product-rating--alignment-fix',
				'login--extra-login-controls',
				'posts-base--load-more-masonry-fix',
				'posts-base--display-categories',
				'heading--is-bubble',
				'button--icon-style-section',
				'woocommerce-products--horizontal-layout',
		...
		if ( function_exists( 'register_nav_menus' ) ) {
			register_nav_menus(
				array(
					'primary-menu'     => esc_html__( 'Menu Header', 'jolie' ),
				)
			);
		}
	}
	...
	public static function load_languages() {
		load_theme_textdomain( 'jolie', VAMTAM_THEME_DIR . 'languages' );
	}
	...
	public static function license_valid() {
		...
		esc_html_e( 'Valid Purchase Key.', 'jolie' );
		echo '<p>';
		esc_html_e( 'Congratulations! You have succesfully registered your product.', 'jolie' );
		echo '</p>';
		echo '</span>';
	}
	...
	public static function license_invalid() {
		echo '<span id="fail">';
		esc_html_e( 'Incorrect Purchase Key.', 'jolie' );
		echo '<p>';
		esc_html_e( 'Please check your purchase code and re-enter.', 'jolie' );
		echo '</p>';
		echo '</span>';
	}
	...
	public static function license_failed() {
		...
		esc_html_e( 'Cannot validate Purchase Key. Please try again later. If the problem persists your server might not have the curl PHP extension enabled.', 'jolie' );
		echo '</span>';
	}
				...
				<div class="vamtam-license-notice notice">
					...
					<div class="vamtam-notice-content">
						...
						<?php if ( $is_token ) : ?>
							...
							<?php
								echo wp_kses( sprintf( __( ' <a href="%s" target="_blank"> this link</a>', 'jolie' ), esc_url( 'https://api.extensions.envato.com/extensions/begin_activation'
	...
	public static function token_unregistered() {
		echo '<span id="success">';
		esc_html_e('Unregistered Token.', 'jolie');
		echo '</span>';
	}

	public static function token_unregister_fail() {
		echo '<span id="fail">';
		esc_html_e('Failed to unregister token. Please try again later. If this issue persists please contact support.', 'jolie');
		echo '</span>';
	}

	public static function license_unregister() {
		echo '<span id="success">';
		esc_html_e( 'Unregistered Key.', 'jolie' );
		echo '</span>';
	}
